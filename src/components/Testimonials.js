    ...
    <div className="w-full min-h-50v md:min-h-70v p-8 bg-s-blue ">
      ...
      <div className="md:w-3/4 w-full md:h-[45vh] pt-3 relative transition mx-auto">
          ...
          return (
            <>
              <div
                ...
                className={
                  cIndex === index
                    ? "w-full h-full transition mx-auto opacity-100  ease-in-out duration-[2s]  text-s-white justify-center flex flex-col items-center"
                    : "absolute left-[-1000%] opacity-0"
                }
              >
                <TestiCard name={val.name} url={val.url} body={val.body} />
              </div>
            </>
          );
        })}
        <div className="absolute top-1/2 left-[-3%] z-10">
          <BsChevronDoubleLeft
            onClick={() => previous()}
            className="text-s-white cursor-pointer ease-in duration-300 hover:text-s-gray "
            size={45}
          />
        </div>
